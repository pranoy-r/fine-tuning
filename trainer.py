...
def validate(
    ...
    if accelerator.is_main_process:
        print(
            f"Validation Results - Loss: {avg_loss:.4f} | Perplexity: {perplexity:.4f}"
        )

        accelerator.log(
            {"val/loss": avg_loss, "val/perplexity": perplexity}, step=global_step
        )
...
def resume_from_checkpoint(
    ...
    if os.path.exists(weights_path):
        ...
        logging.warning(
            f"No adapter weights found in {checkpoint_dir}. Model remains in base state."
        )

    logging.info(
        f"Resumed optimizer/scheduler state from: {filename} at step {global_step}"
    )
    return global_step
...
class QwenTrainer:
    def __init__(
        ...
        self.model, self.optim, self.scheduler, self.train_dl, self.val_dl = (
            self.accelerator.prepare(model, optim, scheduler, train_dl, val_dl)
        )
        ...
        effective_steps_per_epoch = math.ceil(
            len(self.train_dl) / gradient_accumulation_steps
        )
        effective_training_steps = num_epochs * effective_steps_per_epoch

        logging.info(
            f"Effective batch size per device: {batch_size * gradient_accumulation_steps}"
        )
            ...
            with tqdm(
                ...
                for batch in train_dl:
                    ...
                    if self.accelerator.sync_gradients:
                        # Checkpointing
                        if (
                            not (self.global_step % self.ckpt_every)
                        ...
                        # Sampling
                        if (
                            not (self.global_step % self.sample_every)
                            and self.global_step != 0
                        ):
                            if self.accelerator.is_main_process:
                                sample_generations(
                        ...
                        # Validation
                        if (
                            not (self.global_step % self.eval_every)
                            and self.global_step != 0
                        ):
                            validate(
                                self.model,
...
if __name__ == "__main__":
    ...
    # Project / Dataset
    parser.add_argument(
        "--project_name", type=str, default="Qwen-Finetune", help="WandB project name"
    )
    parser.add_argument(
        "--dataset_path",
        type=str,
        default="my_data.jsonl",
    ...
    parser.add_argument(
        "--weight_decay", type=float, default=0.01, help="Adam weight decay"
    )
    parser.add_argument(
        "--max_grad_norm",
        type=float,
    ...
    parser.add_argument(
        "--ckpt_every", type=int, default=500, help="Save checkpoint every N steps"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=250,
        help="Sample and log text every N steps",
    )
    parser.add_argument(
        "--ckpt_saved_dir",
        type=str,
        default="qwen_checkpoints",
    ...
    steps_per_epoch = max(
        1, math.ceil(len(train_dl) / args.gradient_accumulation_steps)
    )
